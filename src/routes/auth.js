...
export async function handleAuth(request, env, path) {
  ...
  if (path === '/api/auth/login' && method === 'POST') {
    try {
      ...
      try {
        ...
        console.error('JWT 발급 에러:', jwtErr);
        return json({ error: '토큰 발급 에러: ' + jwtErr.message }, 500);
      }

      // 성공 응답 (profile_image 포함)
      return json({
        token,
        user: {
          id:            user.id,
  ...
  if (path === '/api/auth/signup' && method === 'POST') {
    try {
      ...
      if (!email || !password || !name) {
        return json({ error: '필수 항목을 모두 입력하세요.' }, 400);
      }
  ...
  if (path === '/api/auth/me' && method === 'GET') {
    try {
      ...
      const dbUser = await env.DB.prepare(
        `SELECT u.id, u.username, u.email, u.name, u.role, u.company_id, u.is_active, u.profile_image,
  ...
  if (path === '/api/auth/companies-public' && method === 'GET') {
    try {
      ...
      console.error('관계사 목록 조회 에러:', err);
